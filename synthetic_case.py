...
# We select a subdomain of 37 by 37 grid points. The goal is to make this case similar to the southern Ghana case (in term of size)
u_domain = np.loadtxt(IMERG_file, delimiter=',').reshape((nt,nx,ny))[:,14:-14,14:-14]
lon_u = np.array(np.arange(-4.75, 1.75, 0.1))[14:-14]
lat_u = np.array(np.arange(3.25, 9.75, 0.1))[14:-14]
...
v_domain = np.loadtxt(TAHMO_file, delimiter=',').reshape((nt,nx,ny))[:,14:-14,14:-14]
...
u_warped = mapped(u_dom_resample.reshape(ntr,nx*ny), t_resample, Tt_grid, I).reshape(ntr, nx, ny)
...
if stats:
    ...
    RMSE_before = np.sqrt(np.mean((u_dom_resample - v_dom_resample) ** 2))
    MAE_before = np.mean(np.abs(u_dom_resample - v_dom_resample))
    C_before = np.corrcoef(u_dom_resample.reshape(-1), v_dom_resample.reshape(-1))[0, 1]

    # Statistics after warping
    RMSE_after = np.sqrt(np.mean((u_warped - v_dom_resample) ** 2))
    MAE_after = np.mean(np.abs(u_warped - v_dom_resample))
    C_after = np.corrcoef(u_warped.reshape(-1), v_dom_resample.reshape(-1))[0, 1]
...
if plot:
    ...
    for ktr in range(0,ntr-1,10):
        kt = int(ktr/10)
        if kt == 12:
            av = axarr[int(pos / 6), pos % 6].pcolormesh(lon_o, lat_o, u_warped[ktr, :, :], cmap=my_cmap, norm=norm)
        else:
            axarr[int(pos / 6), pos % 6].pcolormesh(lon_o, lat_o, u_warped[ktr, :, :], cmap=my_cmap, norm=norm)
